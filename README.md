...
compressor 동력 이론값 ~1.14 MW  

압축기 소비 동력 방정식 (열역학 제1법칙):
W_comp = ṁ × cp × (T2 - T1)  

각 값 대입:  

ṁ    = 3.493 kg/s       (TPE331-10 설계점 공기유량)  

cp   = 1005 J/(kg·K)    (공기 기준, γ=1.4, R=287.04)  

T2   = 613.75 K         (compressor1.t_out 시뮬레이션 값)  

T1   = 288.15 K         (inlet1.t_out 시뮬레이션 값)  


W_comp = 3.493 × 1005 × (613.75 - 288.15)
       = 3.493 × 1005 × 325.6
       = 1,142,600 W
       ≈ 1.14 MW
...
여기서 cp는 연소가스 기준 (γ=1.31, R=287.04):  

cp = γR / (γ-1) = 1.31 × 287.04 / 0.31 = 1212.7 J/(kg·K)  

각 값 대입:  

ṁ    = 3.493 kg/s       (설계점 공기유량)  

cp   = 1212.7 J/(kg·K)  (연소가스 기준)  

T3   = 1250 K           (turbine1.t_in = TIT)  

T4   = 924.87 K         (turbine1.t_out 시뮬레이션 값)  


W_turb = 3.493 × 1212.7 × (1250 - 924.87)  
