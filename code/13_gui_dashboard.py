...
def render_sweep_figure_png(mode: str, value: float, y_col: str, y_label: str, slider_label: str) -> bytes:
    ...
    if mode == "eta_b":
        ...
        # 걸 시각적으로 구분한다.
        for lo_b, hi_b, band_label, color in (
        ...
        for lo_z, hi_z, zone_label, color in (
            (0.85, 1.00, "정상 인접(참고)", "#cdeccd"),
            (0.60, 0.85, "moderate 인접(참고)", "#ffd08a"),
            (0.30, 0.60, "severe 인접(참고)", "#ffb3a8"),
        ):
...
def render_panel(col, title: str, slider_label: str, slider_key: str,
    ...
    with col:
        ...
        stage_label = f"{slider_label}={value:.3f}"   # 더 이상 이름 붙은 단계가 아니라 실제 값으로 표시
